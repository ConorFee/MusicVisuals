...
public class JVisual extends Visual {
    ...
    float circleSize; // put circlesize up here as i was having trouble accessing it in the clouds
                      // function
    ...
    public void draw() {
        ...
        // Call methods with updated parameters
        drawMountains(avg);
        ...
        // going to attempt to make similar lines to this
        // https://www.youtube.com/watch?v=E9SD8M_awps
    ...
    private void setGradient(int x, int y, float w, float h, int c1, int c2) { // used for the fading sky, draws lines
                                                                               // repeatedly, slightly changing colour
                                                                               // as it increments
        noFill();
        for (int i = y; i <= y + h; i++) {
    ...
    public void clouds() {
        fill(255);
        noStroke();
        int DECRET = 30; // variable i use to decrease the size of the clouds/cirlces as it uses the same
                         // sizing as the sun
    ...
    public void cloudShadow() {
        ...
        for (PVector pos : cloudPostions) {
            ...
            // ellipse(pos.x, pos.y, circleSize - DECRET, circleSize - DECRET);
            // ellipse(pos.x + random(-20, 20), pos.y + random(-20, 20), circleSize -
            // DECRET, circleSize - DECRET);
            // ellipse(pos.x + random(-20, 20), pos.y + random(-20, 20), circleSize -
            // DECRET, circleSize - DECRET); //this was an attempt to randomize the shape of
            // each cloud when clicked but it reset evey frames
            ellipse(pos.x, pos.y, circleSize - DECRET, circleSize - DECRET);
            ellipse(pos.x - 20, pos.y + 0, circleSize - DECRET, circleSize - DECRET);
            ellipse(pos.x + 5, pos.y - 5, circleSize - DECRET, circleSize - DECRET);
            ellipse(pos.x + 2, pos.y + 5, circleSize - DECRET, circleSize - DECRET);
